...
export default class Api extends BaseCommand {
  ...
  static override examples = [
    ...
    'agledger api POST /v1/records --data \'{"type":"notarize-generic-v1","criteria":{"task_description":"..."}}\'',
    'agledger api POST /v1/records -F type=notarize-generic-v1 -F criteria.task_description="summarize Q3 filings"',
  ...
  static override args = {
    ...
    path: Args.string({
      description: 'Full API path including version prefix (e.g. /v1/records, /health, /federation/v1/peer)',
      required: true,
    }),
  };
    ...
    try {
      ...
      if (!args.path.startsWith('/')) {
        this.failWith(
          ...
          ExitCode.USAGE_ERROR,
          'Use the full path including version prefix, e.g. /v1/records or /health. Run `agledger discover` for the workflow.',
        );
      }

      // Reject control characters before building the request URL. Agent-supplied
      // paths are untrusted input; control chars enable request-line / header
      // injection and never appear in a legitimate API path.
      if (/[\x00-\x1f\x7f]/.test(args.path)) {
        this.failWith(
          ErrorCode.INVALID_PATH,
