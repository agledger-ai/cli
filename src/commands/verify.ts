...
import { verifyExport, type VerifyExportResult, type RecordAuditExport } from '../verify/verify-export.js';
import { BaseCommand, ErrorCode, ExitCode } from '../base.js';

/**
 * Offline verification of a record audit export. Runs entirely offline — no
...
export default class Verify extends BaseCommand {
  static override description =
    'Verify a record audit export offline (hash chain + Ed25519 signatures, RFC 8785).';
  ...
  async run(): Promise<void> {
    ...
    const exportData = this.readJsonSource(args.file, 'audit export') as RecordAuditExport;
