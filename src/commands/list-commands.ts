...
export default class ListCommands extends BaseCommand {
  ...
  async run(): Promise<void> {
    ...
    const commands = [
      ...
      { name: 'docs', description: 'Fetch the API\'s agent-oriented docs narrative (llms.txt / llms-full.txt).' },
      ...
      { name: 'verify', description: 'Verify an audit export offline (hash chain + envelope signatures).' },
    ...
    this.output({
      commands,
      note: 'For API operations (records, completions, schemas, webhooks, etc.), use `agledger api <METHOD> <path>`. See `agledger discover` for the workflow.',
    });
  }
}
