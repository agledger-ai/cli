...
describe('agledger api: method + path validation', () => {
  it('rejects unknown method', () => {
    const result = run('api FROGGY /v1/records --json', { AGLEDGER_API_KEY: 'ach_ent_test' });
    expect(result.exitCode).toBe(2);
    const parsed = parseJson(result);
  ...
  it('rejects path without leading /', () => {
    const result = run('api GET v1/records --json', { AGLEDGER_API_KEY: 'ach_ent_test' });
    expect(result.exitCode).toBe(2);
    const parsed = parseJson(result);
  ...
  it('normalizes lowercase method', () => {
    const result = run('api get /v1/records --dry-run --json', { AGLEDGER_API_KEY: 'ach_ent_test' });
    expect(result.exitCode).toBe(0);
    const parsed = JSON.parse(result.stdout);
...
describe('agledger api: --data body handling', () => {
  it('accepts --data as JSON body on POST', () => {
    const result = run(
      'api POST /v1/records --data \'{"type":"ACH-PROC-v1"}\' --dry-run --json',
      { AGLEDGER_API_KEY: 'ach_ent_test' },
    );
    expect(result.exitCode).toBe(0);
    const parsed = JSON.parse(result.stdout);
    expect(parsed.body).toEqual({ type: 'ACH-PROC-v1' });
  });
...
describe('agledger api: --input file and stdin', () => {
  it('reads JSON body from --input file', () => {
    const file = tmpJson({ type: 'ACH-DATA-v1', criteria: { x: 1 } });
    const result = run(`api POST /v1/records --input ${file} --dry-run --json`, {
      AGLEDGER_API_KEY: 'ach_ent_test',
    });
    expect(result.exitCode).toBe(0);
    const parsed = JSON.parse(result.stdout);
    expect(parsed.body).toEqual({ type: 'ACH-DATA-v1', criteria: { x: 1 } });
  });
  ...
  it('reads from stdin when --input is -', () => {
    const stdout = execSync(
      `node ${BIN} api POST /v1/records --input - --dry-run --json`,
      {
        ...
        input: '{"type":"ACH-ORCH-v1"}',
        timeout: 10_000,
      },
    );
    const parsed = JSON.parse(stdout.trim());
    expect(parsed.body).toEqual({ type: 'ACH-ORCH-v1' });
  });
});
